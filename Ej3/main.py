...
Los 3 lugares son:
    ...
    """
    output_widget.insert('end', "Los 3 lugares son: \
...
def create_gui():
    window = Tk()
    window.title("Redis Análisis Geoespacial")

    window.geometry("800x600")

    import_csv_to_redis()

    Label(window, text="Elija una opción:", font=("Roboto", 20)).pack()

    Button(window, text="¿Cuantos viajes se generaron a 1km de distacia de los 3 lugares mencionados?", font=("Roboto", 14), command=lambda: handle_option_b(output_widget)).pack()
    Button(window, text="¿Cuantas KEYS hay en la base de datos Redis?", font=("Roboto", 14), command= lambda: handle_option_c(output_widget)).pack()
    Button(window, text="¿Cuantos miembros tiene la key 'bataxi'?", font=("Roboto", 14), command=lambda: handle_option_d(output_widget)).pack()
    Button(window, text="¿Sobre qué estructura de Redis trabaja el GeoADD", font=("Roboto", 14), command=lambda: handle_option_e(output_widget)).pack()

    output_widget = Text(window, wrap='word', font=("Roboto", 12))
